...
str(mds)
g1 <- ggplot(mds, aes(X1, X2, label = city)) + geom_point() +
  geom_text()

g2 <- ggplot(mds, aes(X1, X2, label = city)) + geom_text()
...
admission <- UCBAdmissions


str(UCBAdmissions)
admission <- as.data.frame(UCBAdmissions)


library(ggplot2)

## g3 <- ggplot(admission, aes(Gender, Freq, fill = Admit)) + facet_wrap(Dept ) + geom_col() + scale_fill_manual(values = c'admitted' = green )
